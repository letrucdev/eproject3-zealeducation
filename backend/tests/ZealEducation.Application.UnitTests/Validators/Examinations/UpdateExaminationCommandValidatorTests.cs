...
public class UpdateExaminationCommandValidatorTests
{
    private readonly UpdateExaminationCommandValidator _validator = new();

    private static UpdateExaminationCommand Valid(
        ...
        int passScore = 50) => new(
            ...
            examName,
            examDate ?? new DateOnly(2030, 5, 20),
            location,
            maxScore,
            passScore);
    ...
    {
        var result = _validator.TestValidate(Valid(examName: new string('B', 101)));
        result.ShouldHaveValidationErrorFor(c => c.ExamName);
    }
    ...
    {
        var result = _validator.TestValidate(Valid(examName: new string('B', 100)));
    ...
    {
        var result = _validator.TestValidate(Valid(location: new string('y', 101)));
    ...
    {
        var result = _validator.TestValidate(Valid(maxScore: 50, passScore: 51));
    ...
    {
        var result = _validator.TestValidate(Valid(maxScore: 80, passScore: 80));
        result.ShouldNotHaveAnyValidationErrors();
    }
}
