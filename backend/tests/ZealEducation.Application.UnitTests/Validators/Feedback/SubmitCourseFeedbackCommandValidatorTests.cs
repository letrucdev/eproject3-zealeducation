...
public class SubmitCourseFeedbackCommandValidatorTests
{
    private readonly SubmitCourseFeedbackCommandValidator _validator = new();

    private static SubmitCourseFeedbackCommand Valid(
        Guid? batchId = null,
        int rating = 4,
        string? comment = "Good course") => new(
    ...
    {
        var result = _validator.TestValidate(Valid(comment: new string('y', 1001)));
    ...
    {
        var result = _validator.TestValidate(Valid(comment: new string('y', 1000)));
